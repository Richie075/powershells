param(
    [Parameter(Mandatory=$true)][int]$olderThen
)
...
and DATEDIFF(Day, Convert(varchar(10), porh.TimeStamp, 111), Convert(varchar(10), GetDate(), 111)) > {0} 
...
Select Count(cpu.Id) AS 'CPUs To Delete' From CodePoolUSC cpu with (nolock) JOIN USC u ON cpu.SerialNumber = u.Number JOIN @poIds pid ON u.POID = pid.Id", $olderThen)
 #exec spPO_SN_List @poNumber = {0}, @lotNumber = '{1}', @excludeDeletedPOs = {2}
Invoke-Sqlcmd -ServerInstance $SQLServer -Database $db3 -Query $selectdata -username sa -password Picture04 -QueryTimeout 70000 | Export-CSV $AttachmentPath
}
